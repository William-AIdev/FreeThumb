...
| “关屏”和“锁屏”被错误地当成同一件事 | 放弃会触发锁屏策略的 `displaysleepnow`；合盖仅调整内屏背光；主动锁屏使用独立流程 | Windows 可直接调用官方 `LockWorkStation`，并同时维持 power request。锁屏按钮与显示器/合盖逻辑必须解耦。 |
...
3. 使用 `LockWorkStation` 实现真正的一键 `Lock & Keep Running`。
4. 读取 AC/电池、低电量和系统电源事件，复用现有告警语义。
5. 显示当前睡眠模型和合盖能力诊断；首版不自动修改合盖策略。
6. 完成真实硬件矩阵后，再决定是否提供用户确认后的合盖策略向导。
